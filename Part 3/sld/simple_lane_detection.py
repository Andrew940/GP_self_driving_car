...
def draw(lines, image, color=[255, 0, 0], thickness = 5, thresh = 0.5):
                ...
                if filtered_lines is not None:
                    for filtered_line in filtered_lines:
                        for x1, y1, x2, y2 in filtered_line:
                            x1_mean = int((x1 + x1_mean)/2)
                            y1_mean = int((y1 + y1_mean)/2)
                            x2_mean = int((x2 + x2_mean)/2)
                            y2_mean = int((y2 + y2_mean)/2)

    line_mean = (x1_mean,y1_mean,x2_mean,y2_mean)

    cv2.line(lined_image, (x1_mean, y1_mean), (x2_mean, y2_mean), color, thickness)

        
    return lined_image, line_mean
